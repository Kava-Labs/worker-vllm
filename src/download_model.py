...
def download(name, revision, type, cache_dir):
    if type == "model":
        ...
        custom_allow_patterns = os.getenv("CUSTOM_ALLOW_PATTERNS")
        if custom_allow_patterns:
            custom_allow_patterns = custom_allow_patterns.split(",")
            pattern_sets = [custom_allow_patterns]
    elif type == "tokenizer":
        ...
        for pattern_set in pattern_sets:
            ...
            for pattern in pattern_set:
                ...
                if paths:
                    ...
                    # If .gguf file is present, the returned path MUST be the
                    # full file path, not just the directory containing the file
...
if __name__ == "__main__":
    ...
    metadata.update(
        {"TOKENIZER_NAME": tokenizer_path, "TOKENIZER_REVISION": tokenizer_revision}
    )
